function setupRequestPanelModal() {
  ...
  openButton.addEventListener('click', openRequestsPanel);
  closeButton.addEventListener('click', closeRequestsPanel);
  backdrop.addEventListener('click', (event) => {
    if (event.target === backdrop) closeRequestsPanel();
  });

  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && backdrop.classList.contains('open')) closeRequestsPanel();
  });
}
