...
function latestWeekFromData(turni){var keys=Object.keys(turni||{}).filter(function(key){return key.indexOf('-W')>-1});keys.sort();return keys.length?keys[keys.length-1]:''}
function renderReadOnly(data){var root=document.getElementById('readSchedule');var label=document.getElementById('readWeekLabel');if(!root)return;var turni=data&&data.turni?data.turni:{};var week=data&&data.settimana?data.settimana:latestWeekFromData(turni);var weekData=week&&turni[week]?turni[week]:{};if(label)label.textContent=week?'Settimana '+week:'Turni non ancora pubblicati';if(!week||!Object.keys(weekData).length){root.innerHTML='<div class="empty-box">Turni non ancora pubblicati dall’amministratore.<br>Controlla più tardi oppure premi Aggiorna.</div>';return}var html='';readDepartments.forEach(function(dep){html+='<section class="department-card"><div class="department-title">'+dep.title+'</div>';dep.people.forEach(function(person){html+='<div class="employee-row"><div class="employee-name">'+person+'</div><div class="days-grid">';readDays.forEach(function(day){var value=weekData[readCellKey(dep.key,person,day[0])]||'';html+='<div class="shift-cell'+readShiftClass(value)+'"><span class="shift-day">'+day[1]+'</span><span class="shift-value">'+(value||'-')+'</span></div>'});html+='</div></div>'});html+='</section>'});root.innerHTML=html}
